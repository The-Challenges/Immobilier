...
const RequestsList = () => {
  ...
  const fetchRequests = async (userId, savedStatuses) => {
    try {
      const response = await axios.get(`http://192.168.103.20:4000/api/request/seller/${userId}/house`);
  ...
  const handleStatusChange = async (id, status) => {
    try {
      await axios.post(`http://192.168.103.20:4000/api/request/updateStatus/${id}`, { status });
