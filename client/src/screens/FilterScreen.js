...
const FilterScreen = ({ navigation }) => {
  ...
  const fetchOptions = async (type, setOptions) => {
    try {
      const response = await axios.get(`http://192.168.103.4:4000/api/${type}/all${type}`);
  ...
  const fetchFilteredHouses = async () => {
    try {
      ...
      const response = await axios.get('http://192.168.103.4:4000/api/house/filterhouses', { params });
