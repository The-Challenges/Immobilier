...
const SeeAllHouses = ({ navigation }) => {
  ...
  const fetchFavorites = async (userId) => {
    if (!userId) return;
    try {
      const response = await axios.get(`http://192.168.104.11:4000/api/favorites/${userId}/house`);
  ...
  const toggleFavorite = async (houseId) => {
    ...
    setLoading(true);
    try {
      await axios.post(`http://192.168.104.11:4000/api/favorite/toggle`, { userId, estateId: houseId, type: 'house' });
      setFavorites(prev => {
